...
DST=jboss16.stage.fin
DST3=search16.stage.fin

echo "AGGIORNAMENTO della distribuzione su server di stage" | tee ${LOG} 2>${ERR}

echo "Nuova dist in ${NEWDIST}" | tee -a ${LOG} 2>${ERR}
ls -l ${NEWDIST} | tee -a ${LOG} 2>${ERR}

echo "RINOMINO la cartella ${DEST} in ${DESTBK} su stage" | tee -a ${LOG} 2>${ERR}
ssh -i ${SEC} ${DST} "mv ${DEST} ${DESTBK}" | tee -a ${LOG} 2>${ERR}

echo "COPIO la nuova distribuzione compilata su stage" | tee -a ${LOG} 2>${ERR}
