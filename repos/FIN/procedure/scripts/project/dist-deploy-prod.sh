...
ssh -i ${SEC} search16.prod.fin '/usr/sbin/service tomcat_int stop' | tee -a ${LOG} 2>${ERR}
ssh -i ${SEC} search16.prod.fin '/usr/sbin/service tomcat_solr stop' | tee -a ${LOG} 2>${ERR}

cd ${NEWDIST}

ssh -i ${SEC} search16.prod.fin "cd /opt/tomcat_solr/webapps ; rm -rf solr solr-indexer ; ls -ltr" | tee -a ${LOG} 2>${ERR}
ssh -i ${SEC} search16.prod.fin "cd /opt/tomcat_int/webapps ; rm -rf integration-server ; ls -ltr" | tee -a ${LOG} 2>${ERR}

ssh -i ${SEC} search16.prod.fin "cd /opt/tomcat_solr/logs ; mkdir -p backup ; mv *.* backup  ; ls -ltr" | tee -a ${LOG} 2>${ERR}
ssh -i ${SEC} search16.prod.fin "cd /opt/tomcat_int/logs ; mkdir -p backup ; mv *.* backup  ; ls -ltr" | tee -a ${LOG} 2>${ERR}

scp -i ${SEC} deployment-servers/solr.war deployment-servers/solr-indexer.war  search16.prod.fin:/opt/tomcat_solr/webapps | tee -a ${LOG} 2>${ERR}

scp -i ${SEC} deployment-servers/integration-server.war search16.prod.fin:/opt/tomcat_int/webapps 2>&1 | tee -a ${LOG}
...
ssh -i ${SEC} search16.prod.fin '/usr/sbin/service tomcat_int start' | tee -a ${LOG} 2>${ERR}
